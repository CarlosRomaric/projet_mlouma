...
@include('livewire.projets.projetmodal')
   
            <label for="" class="text-4xl font-bold">Liste des Projets</label>
            <div class="flex flex-col sm:flex-row mt-2 w-full justify-between">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center mt-2 sm:mt-0 sm:order-2">
                    ...
                    <div class="flex flex-col sm:flex-row mt-2 sm:mt-0 sm:ml-2 w-full sm:w-auto">
                        ...
                        <button class="btn-green-table flex items-center w-full sm:w-auto mt-2 sm:mt-0 sm:ml-2 " data-te-toggle="modal" data-te-target="#projetModal" data-te-ripple-init data-te-ripple-color="light" wire:click="create"> 

                            <img src="{{ asset('assets/img/icons/add.svg') }}" alt="" class="w-5 pr-2">
                            <label for="" class="cursor-pointer">Ajouter un Projet</label>
                        </button>
                    </div>
                </div>
            </div>
                        ...
                        <div class="overflow-hidden">
                            <table class="min-w-full text-left text-sm font-light my-10">
                                <thead class="bg-green-custom bt-table">
                                    ...
                                    <th scope="col" class="px-6 py-4">Localisation</th>
                                ...
                                <?php $i=0;?>
                                    ...
                                    <tr class="border-b border-t-2 border-green-900 {{ $i % 2 !== 0 ? '' : 'bg-green-100' }} dark:border-green-900">
                                        <td class="whitespace-nowrap px-6 py-4 font-medium" wire:key="{{ $projet->id }}">{{ $i }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $projet->name }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $projet->location }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">
                                            <button class="btn-success" wire:click='edit("{{$projet->id}}")' >
                                            ...
                                            <button class="btn-danger flex flex-row" wire:click='deleteForm("{{$projet->id}}")' >
                                                <svg width="15px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <button wire:click='show("{{ $projet->id }}")' class="btn-success mb-0 flex flex-row">
                            ...
                            <div class="livewire-pagination">{{ $projets->links('custom-pagination-links') }}</div>
