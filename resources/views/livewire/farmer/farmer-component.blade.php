<div>
    ...
    <div class="flex flex-col sm:flex-row mt-2 w-full justify-between">
        ...
        <div class="mb-2 sm:mb-0 sm:flex-grow w-full sm:w-60 sm:order-1">
            <input type="text" class="bg-green-100 w-30 rounded-lg shadow px-4 py-2 mt-2" wire:model.live="search" placeholder="Saisir pour rechercher">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center mt-2 sm:mt-0 sm:order-2">
            ...
            <div class="py-1 px-4 text-center sm:text-right">
                <label for="">Nombre d'enregistrement : {{ $farmers->count() }} </label>
            </div>
            ...
            <div class="flex flex-col sm:flex-row mt-2 sm:mt-0 sm:ml-2 w-full sm:w-auto">
                ...
                <a href="{{ route('farmers.import.create') }}" class="btn-green-table flex items-center w-full sm:w-auto">
                ...
                <a href="{{ route('farmers.export') }}" class="btn-green-table flex items-center w-full sm:w-auto mt-2 sm:mt-0 sm:ml-2">
                ...
                <button  class="btn-green-table flex items-center w-full sm:w-auto mt-2 sm:mt-0 sm:ml-2" wire:click="create" >
            ...
            <div class="overflow-hidden">
                <table class="min-w-full text-left text-sm font-light my-10">
                    <thead class="border-b bg-green-custom text-white font-medium dark:border-green-custom">
                        ...
                        <th scope="col" class="px-6 py-4">Date et lieu de naissance</th>
                        ...
                        <th scope="col" class="px-6 py-4">Departement</th>
                        <th scope="col" class="px-6 py-4">Localités</th>
                        <th scope="col" class="px-6 py-4">Nb. parcelle</th>
                    ...
                    <?php $i=0;?>
                        ...
                        <tr class="border-b border-t-4 border-green-900 dark:border-green-900">
                            ...
                            <td class="whitespace-nowrap px-6 py-4">{{ $farmer->fullname }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $farmer->born_date }} à {{ $farmer->born_place }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $farmer->phone }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $farmer->sexe }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ ($farmer->agribusiness)  ? $farmer->agribusiness->name : '' }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ ($farmer->region)  ? $farmer->region->name : '' }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ ($farmer->departement)  ? $farmer->departement->name : '' }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $farmer->locality }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $farmer->number_of_plots}}</td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    <!-- <button class="btn-success" wire:click='edit("{{ $farmer->id }}")' >
                                    ...
                                    <a href="{{ route('plots.index', $farmer->id) }}" class="btn-warning">
                                               
                                                <svg width="15px" style="width: 1.2734375em; height: 1em;vertical-align: middle;fill: currentColor;overflow: hidden;" viewBox="0 0 1304 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <a href="{{ route('farmers.edit', $farmer->id) }}" class="btn-success " wire:navigate.hover>
                                                <svg width="15px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M21.1213 2.70705C19.9497 1.53548 18.0503 1.53547 16.8787 2.70705L15.1989 4.38685L7.29289 12.2928C7.16473 12.421 7.07382 12.5816 7.02986 12.7574L6.02986 16.7574C5.94466 17.0982 6.04451 17.4587 6.29289 17.707C6.54127 17.9554 6.90176 18.0553 7.24254 17.9701L11.2425 16.9701C11.4184 16.9261 11.5789 16.8352 11.7071 16.707L19.5556 8.85857L21.2929 7.12126C22.4645 5.94969 22.4645 4.05019 21.2929 2.87862L21.1213 2.70705ZM18.2929 4.12126C18.6834 3.73074 19.3166 3.73074 19.7071 4.12126L19.8787 4.29283C20.2692 4.68336 20.2692 5.31653 19.8787 5.70705L18.8622 6.72357L17.3068 5.10738L18.2929 4.12126ZM15.8923 6.52185L17.4477 8.13804L10.4888 15.097L8.37437 15.6256L8.90296 13.5112L15.8923 6.52185ZM4 7.99994C4 7.44766 4.44772 6.99994 5 6.99994H10C10.5523 6.99994 11 6.55223 11 5.99994C11 5.44766 10.5523 4.99994 10 4.99994H5C3.34315 4.99994 2 6.34309 2 7.99994V18.9999C2 20.6568 3.34315 21.9999 5 21.9999H16C17.6569 21.9999 19 20.6568 19 18.9999V13.9999C19 13.4477 18.5523 12.9999 18 12.9999C17.4477 12.9999 17 13.4477 17 13.9999V18.9999C17 19.5522 16.5523 19.9999 16 19.9999H5C4.44772 19.9999 4 19.5522 4 18.9999V7.99994Z" fill="#fff"/>
                                                </svg>
                                    </a>
                                    <button class="btn-danger " wire:click='deleteForm("{{$farmer->id}}")' >
                                        <svg width="15px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="livewire-pagination">{{ $farmers->links('custom-pagination-links') }}</div>
            </div>
            </div>
        </div>
    </div>
