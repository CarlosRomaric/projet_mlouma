<div class="">
        ...
        @include('livewire.agribusinesses.agribusinessmodal')

    
            <label for="" class="text-4xl font-bold">Liste des Coopératives</label>
            <div class="flex flex-col sm:flex-row mt-2 w-full justify-between">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center mt-2 sm:mt-0 sm:order-2">
                    ...
                    <div class="flex flex-col sm:flex-row mt-2 sm:mt-0 sm:ml-2 w-full sm:w-auto">
                        ...
                        <button class="btn-green-table flex items-center w-full sm:w-auto mt-2 sm:mt-0 sm:ml-2 " data-te-toggle="modal" data-te-target="#agribusinessModal" data-te-ripple-init data-te-ripple-color="light" wire:click="create"> 

                            <img src="{{ asset('assets/img/icons/add.svg') }}" alt="" class="w-5 pr-2">
                            <label for="" class="cursor-pointer">Créer une Coopérative</label>
                        </button>
                    </div>
                </div>
            </div>
                            ...
                            <table class="min-w-full text-left text-sm font-light my-10">
                                <thead class="bg-green-custom bt-table">
                                    <tr class="">
                                        ...
                                        <th scope="col" class="px-6 py-4">Nom de la coopérative</th>
                                        <th scope="col" class="px-6 py-4">Sigle</th>
                                        <th scope="col" class="px-6 py-4">Adresse</th>
                                        <th scope="col" class="px-6 py-4">Region</th>
                                        <th scope="col" class="px-6 py-4">Département</th>
                                        <th scope="col" class="px-6 py-4">Nom du responsable</th>
                                ...
                                <?php $i=0;?>
                                    ...
                                    @forelse($agribusinesses as $agribusiness)
                                        
                                    <?php $i++ ?>
                                    <tr class="border-b border-t-2 border-green-900 {{ $i % 2 !== 0 ? '' : 'bg-green-100' }} dark:border-green-900">
                                        <td class="whitespace-nowrap px-6 py-4 font-medium" wire:key="{{ $agribusiness->id }}">{{ $i }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $agribusiness->name }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $agribusiness->acronym }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $agribusiness->address }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $agribusiness->region->name }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $agribusiness->departement->name }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $agribusiness->person_responsible_name }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">{{ $agribusiness->person_responsible_phone }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">
                                            @if($agribusiness->acronym != 'IDP')
                                            <button class="btn-success" wire:click='edit("{{$agribusiness->id}}")' >
